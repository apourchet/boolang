...
func TestAll(t *testing.T) {
	tree, err := Parse("(A == 1 || B == 2) && (C == 3 || D == 4 && !E == 5) || !(F == 6)")
	assert.Nil(t, err)
	assert.NotNil(t, tree)
...
func TestEvalCount(t *testing.T) {
	...
	counter := 0
	fn := func(l *Leaf, _ ...interface{}) (bool, error) {
		counter += 1
		return true, nil
	}
	tree.Eval(fn)
...
func TestEval(t *testing.T) {
	tree, err := Parse("(false || true) && (true || error && error) || !(error)")
	assert.Nil(t, err)
	assert.NotNil(t, tree)
...
func TestEvalError(t *testing.T) {
	tree, err := Parse("(false || true) && (error || error && error) || !(error)")
	assert.Nil(t, err)
	assert.NotNil(t, tree)
